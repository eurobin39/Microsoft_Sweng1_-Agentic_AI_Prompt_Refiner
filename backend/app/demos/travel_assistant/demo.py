...
from app.demos.travel_assistant.runner import run_sync
from app.models.trace_logs import TraceLog


GROUND_TRUTH_PATH = Path(__file__).parent / "ground_truth.json"
TRACE_DIR = str(Path(__file__).parent / "log" / "traces")
LOG_FILE = str(Path(__file__).parent / "log" / "travel_assistant.log")
...
def run_demo() -> list[TraceLog]:
    header("TRAVEL ASSISTANT DEMO — HANDOFF WORKFLOW")
    ...
    for i, tc in enumerate(test_cases, 1):
        ...
        trace_dict = run_sync(tc["user_request"], log_file=LOG_FILE, trace_dir=TRACE_DIR)
